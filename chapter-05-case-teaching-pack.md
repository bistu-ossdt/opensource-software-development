...
- `tldr-pages/tldr` 对应正文中的行走式阅读演示，用来承接图 5-1，让学生看到“第一轮阅读顺序”如何真正落地。
- VS Code 对应正文中对模板、`CODEOWNERS`、工作流和 Pull Request 结构的分析，用来承接图 5-2 与表 5-1，帮助学生把零散对象重新压缩成项目级判断。
- Linux 对应正文中的“公共证据分布不在同一层”讨论，用来校正读者对陌生项目的阅读预期，并说明为什么长期基础设施项目不能只看仓库首页。
- OpenClaw 对应正文中“现代 AI 原生项目的公共入口与边界判断”讨论，用来帮助读者理解 AI 项目对象的阅读重点为什么与传统项目不同。

## 课堂对象绑定建议

- 图 5-1 固定绑定 `tldr-pages/tldr`：用于演示第一轮阅读顺序。
- 图 5-2 固定绑定 VS Code 与 Linux：用于演示“同样是成熟项目，证据分布层级并不相同”。
- 表 5-1 固定用于四个案例的行动选择比较：为什么 `tldr-pages/tldr` 更适合作为练习对象，而 Linux 更适合作为长期观察对象。
- OpenClaw 只承担“现代 AI 项目边界判断”这条支线，不替代本章的主方法案例。
