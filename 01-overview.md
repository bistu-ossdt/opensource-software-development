...
因此，本章要回答的不是“开源有什么好处”这样单一的问题，而是更基础的几个问题：在“开源”（`open source`）这个词出现之前，软件世界经历了怎样的变化？GNU 与自由软件运动到底提出了什么？Linux、Mozilla、Apache 这些项目为什么具有历史象征意义？为什么今天必须把开源理解为一种由制度、社区和工程共同支撑的软件生产方式？本章不是编年史，也不是著名项目小传，它要建立的是一条解释现代开源世界的起点主线。
...
1985 年，Stallman 发布了较完整版本的《GNU Manifesto》，同年 10 月，自由软件基金会（Free Software Foundation, `FSF`）成立。到这一步，问题已经不仅是“有没有人愿意共享代码”，而是“软件的使用者是否应该拥有运行、研究、修改和再分发软件的自由”。自由软件运动的核心并不在于价格，而在于控制权。这里的 `free` 首先指自由（freedom），而不是免费（free of charge）。
...
这一点对理解开源历史非常关键。今天很多人会把自由软件和开源软件混为一谈，仿佛它们从一开始就是同一个概念。历史上并不是这样。GNU 与 FSF 代表的是自由软件运动，它强调软件自由背后的伦理和制度问题；而“开源”（`open source`）作为术语，以及围绕这一术语形成的组织化推动，则是 1998 年才出现的事情。两者有深刻连续性，但并不完全相同。
...
<div class="history-story">
  ...
  <p>1991 年 8 月 25 日，芬兰的夏天还没有完全过去。21 岁的赫尔辛基大学学生 Linus Torvalds 坐在电脑前，在 <code>comp.os.minix</code> 新闻组里写下一段后来会被无数次转引的话。他说，自己正在为 386(486) AT 机器写一个免费的操作系统，只是出于兴趣，规模也不会像 GNU 那样“大而专业”。今天回头看，这像一段传奇的开场白；但在当时，它更像一个年轻程序员把自己的私人实验轻轻推到公共空间里，带着一点克制，也带着一点试探：我做了个东西，也许还很小，但如果你愿意，可以一起来看。</p>
  <p>真正改变历史的，不是这段话本身，而是它发出之后发生的事。Linux 很快不再只是 Linus 电脑里的一个实验性内核。代码被公开出来，别人开始下载、编译、测试、报告错误、寄回补丁；讨论从“一个人写了什么”慢慢变成“一群人怎样把它做下去”。到 1992 年，Linux 转入 GNU GPL 协作路径，与 GNU 已经完成的大量编译器、shell、库和工具结合，逐步形成了真正可部署的自由系统。1993 年前后，Slackware、Debian 等发行版又把这些零散组件组织成普通人也能安装和运行的系统。故事到这里，已经不再只是“某个学生写出了一个内核”，而是自由软件世界第一次补上了长期缺失的关键一块：理念、工具链和内核终于拼成了可以进入现实计算环境的完整系统。</p>
  <p>Linus 自己也在这个过程中被历史重新塑形。最早，他只是写代码的人；后来，他越来越像一个协调者、裁决者和维护者体系的中心。Linux 内核并不是靠“一位天才不断独写下去”才长大的，而是在邮件列表、补丁评审、子系统维护者和长期协作规则中慢慢长成的。2005 年，在内核协作工具发生变化的背景下，Linus 又写出了 Git；它原本只是为 Linux 内核协作服务的工具，后来却反过来改变了几乎整个软件世界的版本控制与协作方式。再往后，Linux 进入服务器、互联网基础设施、云平台、嵌入式设备、移动终端和超级计算环境，影响早已远远超出“一个操作系统内核”。很多年轻读者今天每天都在间接使用 Linux，却未必意识到，它最初只是一个普通学生在公共网络上发出的一次邀请。Linus 和 Linux 的完整故事之所以重要，正是因为它把开源最动人的那一面展示得非常清楚：开放协作并不是抽象口号，它真的可以从个人兴趣出发，长成支撑全球技术基础设施的公共工程。</p>
</div>
...
Mozilla 的意义因此不应被缩减为浏览器历史的一段插曲。它更像是一个传播窗口：开放源码第一次以大型商业软件事件的形式进入主流讨论空间。对媒体来说，这意味着开源不再只是程序员内部议题；对企业来说，这意味着开放源码可能是一种现实可用的战略选择；对开发者来说，这意味着开放协作开始拥有比自由软件运动早期更广泛的公共可见性。后来“开源”（`open source`）这一术语能在 1998 年迅速扩散，很大程度上也与这一传播窗口被打开有关。
...
如果把人物也放回这条链条里，轮廓会更清楚。Stallman 让“软件自由”为何重要第一次被清楚说出，Linus 让开放协作第一次在可用系统层面获得巨大说服力；到了 1998 年前后，Christine Peterson 提出了 `open source` 这一术语，Eric Raymond 和 Bruce Perens 把它推向更容易传播的公共表达，而 Brian Behlendorf 等早期组织者则让 Apache 这类项目显示出另一种可能：开源社区并不只能围绕单一作者旋转，也可以围绕公共规则、共同维护和长期组织成长。人物在这里的重要性，不在于把历史写成英雄谱，而在于他们分别把问题意识、传播语言、工程实践和组织能力推到了新的阶段。
...
这也让开源社区的基本逻辑开始变得清楚：人们先围绕一个共同问题形成项目，再围绕项目形成维护者、贡献者、使用者和组织者这样的不同角色；角色之间并不是靠私人默契长期协作，而是靠公开讨论、责任分工、版本节奏和共同承认的规则维持。代码当然始终是中心对象，但真正让陌生人能够持续协作的，是围绕代码逐步沉淀出来的公共秩序。后面章节要讲的许可证、治理文件、Issue、Pull Request 和发布流程，其实都属于这套秩序在现代仓库中的具体形态。

## 4. 从 1998 到今天：为什么必须重新理解“开源”

1998 年是一个关键年份，但它的重要性不应被简单理解为“从这一年开始，开源才诞生”。更准确的说法是：这一年，自由软件运动积累下来的问题意识，与新的传播策略、企业开放源码事件和组织化推动叠加在一起，使“开源”（`open source`）成为一个能够迅速扩散的新框架。1997 年，《大教堂与集市》已经为开放式开发模式提供了极具传播力的叙事总结；1998 年初，Netscape 开放浏览器源码，为开放源码进入主流公共视野打开了新的窗口；随后，“开源”（`open source`）这一术语被提出，OSI 开始形成组织化推动。正是在这种“理念积累 + 企业事件 + 新术语 + 组织化”的叠加下，开源才从一个较窄社群中的历史传统，迅速变成更容易扩散的公共框架。

根据 OSI 的官方历史，“开源”（`open source`）这一术语在 1998 年 2 月初的一次策略会议上被确定下来，Christine Peterson 提出了这个词，随后 Eric Raymond 和 Bruce Perens 共同推动了开源促进会（Open Source Initiative, `OSI`）的成立。OSI 的意义在于，它把此前分散的实践与讨论，组织成一个更容易被产业界、媒体和更广泛技术群体接受的表达框架。这个变化并没有抹去自由软件运动的历史，相反，它建立在前一阶段的积累之上，只是强调的重心有所不同。

之所以会出现这种表达变化，并不是因为自由软件传统突然失效，而是因为不同表述会带来不同的传播效果。相较于更强调伦理与自由诉求的“自由软件”（`free software`），“开源”（`open source`）更容易被理解为一种开发方式、一种协作模式和一种可与商业共存的组织方法。它弱化了公众在词义上的误解空间，也使更多企业和媒体愿意进入讨论。历史上真正发生的，不是旧问题被放弃，而是旧问题获得了新的语言入口。

从 1998 年往后看，开源世界的扩张呈现出三个长期趋势。

第一，基金会化。越来越多的重要项目不再只依靠个人魅力或少数开发者维持，而是通过基金会、项目委员会、治理流程和社区规则来获得持续性。基金会并不是开源的全部，但它们提供了托管知识产权、协调治理、组织活动和稳定生态的重要能力。今天如果去看 Linux 基金会（Linux Foundation）旗下云原生计算基金会（Cloud Native Computing Foundation, `CNCF`）的公开项目目录，会发现它管理的并不是单一代码库，而是一组处在 `Sandbox`、`Incubating`、`Graduated` 等不同成熟度阶段的项目。这种可见结构本身就在说明：基金会化处理的已经不只是“谁来继续维护一个仓库”，而是“谁来为一整片生态提供公共连续性”。
...
第二，平台化。到 21 世纪，代码托管平台逐步成为开源协作的关键基础设施。GitHub 在 2008 年上线之后，把仓库、议题、Pull Request、代码评审、发布和社区可见性放在了同一平台里，大幅降低了全球协作的门槛。开源项目并不等于 GitHub 项目，但 GitHub 确实改变了许多人理解和参与开源的方式。以 `kubernetes/kubernetes` 这样的仓库为例，外部人打开同一个入口，就能同时看到源码目录、Issue、Pull Request、Release、Actions 和 Security 等页面；代码、工作项、变更记录和发布边界因此第一次被集中成一个公共界面。也正因为如此，Microsoft 收购 GitHub 才具有强烈的历史意味：这不是一个普通平台并购，而是开源已经从边缘文化走向现代软件基础设施的象征。
...
第三，产业化。今天，大量企业依赖开源、参与开源、维护开源，甚至围绕开源建立商业模式。这里需要再次强调：开源不是反商业。历史真正表明的是，开源与商业之间始终存在张力，但也始终在相互塑造。像 Red Hat 围绕 Linux 发行版、支持服务和企业基础设施形成长期业务这样的案例，已经说明企业参与并不只是外围赞助，而是把开源直接纳入产品、服务和维护体系之中。企业可能借助开源建立生态、扩大标准影响力、吸引开发者，也可能通过基金会、赞助、维护和代码贡献反向塑造开源世界。理解这一点，有助于我们避免把开源讲成一种简单的道德神话。
...
今天读者在一个真实仓库里看到的 `LICENSE`、`CONTRIBUTING.md`、Issue、Pull Request、测试工作流、`CI`、Release 和变更记录，并不是平台时代凭空长出来的新装饰，而是这段历史长期沉淀下来的公共界面。许可证把制度边界写明，贡献说明把进入路径写明，Issue 与 Pull Request 把讨论和修改放到公共记录里，测试与 `CI` 把工程要求变成持续门禁，Release 则把版本承诺公开给外部人。理解这些对象的历史来路，后面章节中的许多“流程”才不会显得像一组孤立规则。

换句话说，开源之所以重要，不是因为它让代码变得“看得见”，而是因为它改变了软件被创造、传播、维护和共同拥有的方式。只有把这一历史脉络看清楚，后面关于许可证、治理、协作流程和项目实践的讨论才不会变成孤立的工具说明。

这也是为什么本章必须在这里收束，而不能继续沿着时间线无限延长。到了这一点，真正重要的问题已经不再是“后来又出现了多少著名项目”，而是：如果开源已经成为一种稳定的软件生产方式，那么它凭什么长期运转？它的制度基础是什么？它的社区和治理如何维持？它的工程流程为何必须公开、可追溯、可协作？因此，后面的章节会按制度基础（第 2 章）、社区与治理（第 3 章）、协作与工程流程（第 4 章）、真实项目阅读（第 5 章），以及参与、AI 辅助与组织发布（第 6-8 章）的顺序继续展开。
