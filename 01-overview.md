...
先看 Linux。到 1990 年前后，GNU 已经完成了大部分关键系统组件，但缺少一个真正可用的内核。1991 年，Linus Torvalds 开始开发 Linux 内核；1992 年，Linux 成为自由软件。GNU 提供的大量系统组件与 Linux 内核结合，才使一个真正可用的自由操作系统环境逐步成形。正因如此，Linux 的历史意义不只是“有人写出了一个内核”，而是它让自由软件系统真正获得了大规模可用性。开源从此不再只是价值立场，也成为可以支撑真实计算环境的基础设施。
