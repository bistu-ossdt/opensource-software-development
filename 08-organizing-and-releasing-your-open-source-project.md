...
与之相对，另一个仓库即使规模不大，只要在首页直接写明“当前是 `v0.x` 实验阶段”、给出最短运行步骤、提供最近一次 Release 和问题报告入口，外部人就能在几分钟内判断自己应继续尝试、继续观察还是暂缓投入。两个仓库都可能公开可见，区别不在代码多寡，而在维护者是否把参与判断所需的关键信息连续地摆到了外部人面前。
...
相反，下面这种写法虽然“有 README”，却几乎没有承担入口职责：
...
当项目本身包含 AI 相关部分时，入口设计还要再多一层说明责任。一个 AI 项目如果公开了代码、参数下载链接或演示页面，维护者就不应让外部人自己猜测这些对象之间是什么关系。更稳妥的做法是直接写清：哪些对象已经公开，哪些对象仍受限，项目当前更接近开放权重（`Open Weights`）、完整的开源人工智能（`Open Source AI`），还是一般开放发布对象；哪些评测、数据说明、已知限制与安全边界需要同时阅读。第 7 章强调了“AI 作为工具进入开源流程”和“AI 项目本身作为对象被判断”这两条线不能混淆；到了第 8 章，维护者要做的正是主动把这种边界写清楚，而不是把判断负担全留给外部人。像 OpenClaw 这类 AI-native 仓库，即使首页已经展示了系统能力和演示效果，维护者仍然要把代码、参数获取方式、已知限制和贡献入口分开写清；否则，外部人看到的就更像效果展示页，而不是参与入口。
...
### Fixed
- add the missing `DATA_DIR` setup step to quickstart (`Closes #42`)
...
像这样的条目，即使只是一条文档修复，也已经把前几章一路出现过的 `DATA_DIR` 问题正式收束为维护者视角下的版本记录：问题不再只是“有人发现过、有人修过”，而是被明确写进了可以对外引用的版本边界里。
...
很多时候，治理能力恰恰体现在这种简短但清楚的回应里。假设项目刚发布 `v1.0`，当前维护重点是修缺陷、补文档和稳定发布线，这时外部人提出“顺便加一个全新的云同步模块”或“把整个配置系统改成插件化”的请求，维护者就需要明确说明：这件事不是永远不讨论，而是不属于当前版本线的承诺边界。

在这种场景下，一个成熟的回复往往会像这样：
...
对前面几章一路推进下来的团队项目来说，发布前最后一次检查也不应只问“代码能否运行”，还应追问：外部人第一次进入时能否按 README 走通，能否从 `CHANGELOG` 判断这版交付了什么，能否知道发现问题后应去哪里报告，以及项目当前准备接住哪些请求、不接住哪些请求。只有这些问题都被回答，团队项目才真正完成了从内部协作产物到公共项目的转换。
