...
<div class="history-story">
  ...
  <p>接下来的几年里，他没有把这件事理解成“某个厂商不够慷慨”，而是把它一步步推向更根本的问题：如果软件越来越重要，那么人们是否还能被排除在理解和修改它的权利之外？`1983-09-27`，Stallman 发布 GNU 计划初始公告，决定写一个自由的、与 Unix 兼容的系统；`1985` 年，他创建 Free Software Foundation，并在《GNU Manifesto》中把这种主张明确表达为 software freedom；到了 `1989-02`，GNU GPL 1.0 发布，这种主张第一次被系统地写成可继承、可分发、可执行的许可证文本。这样回头看，故事的线索非常清楚：它先从一台不能改的打印机开始，后来变成一个系统计划，再后来变成一种制度文本。也正因为这条线索如此清楚，`1998` 年 “open source” 作为新表述出现之后，Stallman 仍一直坚持区分两种说法：他并不否认开放协作的重要性，但始终提醒人们，许可证最初要保护的，不只是更高效的开发方式，而是用户和开发者不应被剥夺的自由。</p>
</div>
