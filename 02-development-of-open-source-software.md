...
自由软件首先是一个关于 software freedom 的概念。按照 GNU / Free Software Foundation 的经典表述，自由软件强调使用者应当拥有四项基本自由：运行程序的自由，研究程序并按自己的需要修改它的自由，再分发副本的自由，以及分发修改版本的自由。FSF 在原始定义里把它们直接编号为 `Freedom 0` 到 `Freedom 3`，对应的是运行（run）、研究并修改（study and change）、再分发副本（redistribute copies），以及分发修改版本（distribute modified versions）。问题不只是“可不可以看到代码”，而是能否运行、研究、修改、再分发和继续传播修改版本。这里的 `free` 首先是 freedom，而不是免费。一个自由软件项目可以收费发行，也可以围绕服务、支持、培训、托管和定制开发形成商业模式。把自由软件理解成“免费软件”，或者理解成“反商业软件”，都属于典型误解。
...
如果进一步看 OSD 的关键条款，会更清楚地发现这一点。OSI 直接把其中若干核心要求写成条款标题，例如 `Free Redistribution`、`Source Code`、`Derived Works` 以及 `No Discrimination Against Fields of Endeavor`。开源不仅要求源代码可获得，还要求自由再分发、允许衍生作品，并且不能对使用者身份或使用领域进行歧视。一个许可证如果禁止商用，或者只允许学术用途，哪怕它把源码完整公开出来，也不符合标准意义上的 open source。正是在这个意义上，“能看到代码”和“这是开源软件”之间，隔着一层明确的制度边界。

需要补充的是，OSI 近年还把这类“什么才算开源”的讨论进一步扩展到了 `AI system`。按照 `Open Source AI Definition` 的口径，判断一个 AI system 是否属于 `Open Source AI`，不能只看模型参数能否下载，而要看支撑修改与再分发所需的 `Data Information`、`Code` 和 `Parameters` 是否一并达到开放条件。也正因为如此，`Open Weights` 并不等于 `Open Source AI`；许多公众语境里被笼统称作“开源模型”的对象，更准确地说往往只是开放权重或一般开放发布。本章在这里先把这条边界立住，后面关于项目阅读、参与路径和发布责任时还会再次回到它。
...
<div class="history-story">
  ...
  <p>接下来的几年里，他没有把这件事理解成“某个厂商不够慷慨”，而是把它一步步推向更根本的问题：如果软件越来越重要，那么人们是否还能被排除在理解和修改它的权利之外？1983 年 9 月 27 日，Stallman 发布 GNU 计划初始公告，决定写一个自由的、与 Unix 兼容的系统；1985 年，他创建 Free Software Foundation，并在《GNU Manifesto》中把这种主张明确表达为 software freedom；到了 1989 年 2 月，GNU GPL 1.0 发布，这种主张第一次被系统地写成可继承、可分发、可执行的许可证文本。这样回头看，故事的线索非常清楚：它先从一台不能改的打印机开始，后来变成一个系统计划，再后来变成一种制度文本。也正因为这条线索如此清楚，1998 年 “open source” 作为新表述出现之后，Stallman 仍一直坚持区分两种说法：他并不否认开放协作的重要性，但始终提醒人们，许可证最初要保护的，不只是更高效的开发方式，而是用户和开发者不应被剥夺的自由。</p>
</div>
