...
自由软件首先是一个关于软件自由（software freedom）的概念。按照 GNU / 自由软件基金会（Free Software Foundation, `FSF`）的经典表述，自由软件强调使用者应当拥有四项基本自由：运行程序的自由，研究程序并按自己的需要修改它的自由，再分发副本的自由，以及分发修改版本的自由。FSF 在原始定义里把它们直接编号为 `Freedom 0` 到 `Freedom 3`，对应的是运行（run）、研究并修改（study and change）、再分发副本（redistribute copies），以及分发修改版本（distribute modified versions）。问题不只是“可不可以看到代码”，而是能否运行、研究、修改、再分发和继续传播修改版本。这里的 `free` 首先指自由（freedom），而不是免费。一个自由软件项目可以收费发行，也可以围绕服务、支持、培训、托管和定制开发形成商业模式。把自由软件理解成“免费软件”，或者理解成“反商业软件”，都属于典型误解。
...
开源软件则更强调另一层表达。按照开源促进会（Open Source Initiative, `OSI`）的定义，开源不是“源码可以看到”这么简单，而是软件的分发条件必须符合开源定义（Open Source Definition, `OSD`）。也就是说，开源首先是一种基于许可证的制度安排。它要求源代码可获得，也要求再分发、衍生作品等关键权利得到明确许可。仅仅把代码放在公开仓库里，并不能自动满足这些条件。

如果进一步看 OSD 的关键条款，会更清楚地发现这一点。OSI 直接把其中若干核心要求写成条款标题，例如 `Free Redistribution`、`Source Code`、`Derived Works` 以及 `No Discrimination Against Fields of Endeavor`。开源不仅要求源代码可获得，还要求自由再分发、允许衍生作品，并且不能对使用者身份或使用领域进行歧视。一个许可证如果禁止商用，或者只允许学术用途，哪怕它把源码完整公开出来，也不符合标准意义上的开源（`open source`）。正是在这个意义上，“能看到代码”和“这是开源软件”之间，隔着一层明确的制度边界。

这种制度边界的讨论并没有停留在传统软件上。随着越来越多人工智能系统以公开仓库、参数下载和开放发布的方式进入公共技术世界，OSI 近年也把“什么才算开源”的判断进一步扩展到了人工智能系统（`AI system`）。按照开源人工智能定义（`Open Source AI Definition`）的口径，判断一个人工智能系统是否属于开源人工智能（`Open Source AI`），不能只看模型参数能否下载，而要看支撑修改与再分发所需的数据信息、代码和参数是否一并达到开放条件。也正因为如此，开放权重（`Open Weights`）并不等于开源人工智能；许多公众语境里被笼统称作“开源模型”的对象，更准确地说往往只是开放权重或一般开放发布。本章在这里先把这条边界立住，后面关于项目阅读、参与路径和发布责任时还会再次回到它。

这种差异也有明确的组织史坐标。1985 年成立的自由软件基金会（Free Software Foundation）用“四项自由”来界定软件自由；1998 年成立的开源促进会（Open Source Initiative）则用开源定义（Open Source Definition）来界定何种分发条件足以构成开源。前者更像是在确认使用者应当拥有哪些基本控制权，后者更像是在确认什么样的许可边界足以支撑开放协作与广泛再分发。
...
这里首先要澄清一个常见误解：开源不是没有版权，恰恰相反，开源依赖版权来进行制度安排。软件作者天然拥有版权，许可证的作用不是消灭版权，而是通过版权授予他人一组明确的权利，同时附带相应条件。也正因为如此，“公开仓库但没有许可证”通常并不意味着别人自动获得了使用、修改和再发布的常规许可。GitHub 官方关于“无许可证”（`No License`）的说明之所以被频繁引用，就是因为它把这个最容易被忽视的事实说得很清楚：代码看得见，不等于你已经拿到了开源协作所需要的那组权利。
...
对今天的项目阅读者来说，这种制度边界在仓库里往往有几个直接可见的入口：根目录的 `LICENSE` 文件，README 中的许可说明，某些项目在再发布时需要一并保留的 `NOTICE`，以及第三方代码或素材的许可列表。GitHub 也会在仓库页面上对可检测的许可证做出可见提示。反过来，如果这些对象都缺失，或者只有模糊的“仅供学习交流”“请勿商用”之类表述，更稳妥的做法就不是默认它“应该算开源”，而是把它视为制度状态仍不清楚的公开仓库。

这也是为什么“源码可见但受限开放”（`source-available`）和“开源”（`open source`）在实践中不能混用。有些项目会把代码公开出来，允许别人阅读，甚至允许有限度地下载和试用，但会在许可证中限制商用、限制特定场景使用，或者不允许形成衍生版本。这类安排当然可能有其项目策略或商业考虑，但它们与符合 OSD 的开源软件不是同一回事。对工程阅读来说，最危险的情况往往不是许可证太复杂，而是读者先入为主地把“公开可见”误判成“可自由协作”。
...
<div class="history-story">
  ...
  <p>这个问题在 Richard Stallman 身上出现得很早，而且最初甚至不是以“许可证条款”的形式出现，而是以一种更直接的失落感出现。二十世纪八十年代初，他在 MIT 人工智能实验室工作时，实验室换上了一台新的激光打印机。过去，程序员会自己修改系统，让打印机卡纸时自动给相关用户发消息；机器出了问题，大家就一起改，改完再继续用，这几乎是实验室再自然不过的合作方式。但这一次，驱动程序的源代码不再像从前那样可以自由研究和修改。后来 Stallman 还得知，别处有人拿到了相关代码，却因为签过保密承诺，不能把副本分享出来。对他来说，这不只是一次“打印机不好用”的烦恼，而像是亲眼看见一扇门被关上了：原本自然发生在程序员之间的理解、修改和分享，开始被制度性地切断。</p>
  <p>接下来的几年里，他没有把这件事理解成“某个厂商不够慷慨”，而是把它一步步推向更根本的问题：如果软件越来越重要，那么人们是否还能被排除在理解和修改它的权利之外？1983 年 9 月 27 日，Stallman 发布 GNU 计划初始公告，决定写一个自由的、与 Unix 兼容的系统；1985 年，他创建自由软件基金会（Free Software Foundation），并在《GNU Manifesto》中把这种主张明确表达为软件自由（software freedom）；到了 1989 年 2 月，GNU GPL 1.0 发布，这种主张第一次被系统地写成可继承、可分发、可执行的许可证文本。这样回头看，故事的线索非常清楚：它先从一台不能改的打印机开始，后来变成一个系统计划，再后来变成一种制度文本。也正因为这条线索如此清楚，1998 年“开源”（`open source`）作为新表述出现之后，Stallman 仍一直坚持区分两种说法：他并不否认开放协作的重要性，但始终提醒人们，许可证最初要保护的，不只是更高效的开发方式，而是用户和开发者不应被剥夺的自由。</p>
</div>
...
理解许可证时，还需要掌握几个基础概念。版权（`Copyright`）是制度基础，它决定作品默认受到保护。分发（`Distribution`）关心的是软件是否被交付或传播给其他人。衍生作品（`Derivative Works`）关注的是修改或再组合后的结果如何处理。声明信息（`Notice`）则通常涉及再发布时需要保留的说明。对于 Apache 这类许可证，还要额外注意专利授权（`Patent Grant`）这一层，因为现代软件项目并不只是单纯的文字作品，它可能还嵌入技术实现上的专利风险。

版权、专利和商标（`Trademark`）也不能被混成一件事。版权决定作品默认受保护，许可证主要通过版权授予使用、修改和再发布的权利；某些许可证会进一步处理专利授权；但项目名称、标识和品牌识别通常属于商标问题，不会因为代码开源就自动一并开放。Apache Software Foundation 的商标政策正好把这一点说得很清楚：即便 Apache-2.0 允许广泛复用代码，许可证本身也不把项目的 trade names、trademarks、service marks 或 product names 一起授予给下游，除非只是为了合理说明作品来源或复制 `NOTICE` 所需。对读者来说，这意味着“可以复用代码”“可以继续分发修改版”和“可以继续沿用原项目名称或 Logo 发布”，并不是同一个问题。
...
先看宽松式许可证。`MIT License` 是最常见的例子之一。它的特点是授权范围广，允许使用、复制、修改、合并、发布、分发、再许可，甚至销售软件，但前提不是“完全没有义务”，而是需要保留版权声明和许可声明，并接受免责条款。`BSD-3-Clause` 也属于宽松式许可证，它与 MIT 很接近，但多了一个很值得记住的约束：不得使用原作者或贡献者的姓名为衍生产品背书。对入门读者来说，把 BSD 讲成“和 MIT 很像，但多了不得背书（`no-endorsement`）条件”，通常已经足够。
...
| BSD-3-Clause | 宽松式 | 类似 MIT，但增加不得背书（`no-endorsement`）要求 | 保留声明，不得用原作者背书 |
...
也正因为如此，许可证选择同时还是一个兼容性问题，而不只是偏好问题。只要项目里引入了第三方库、图片、数据、模型或模板，你就不再只是为“自己的代码”挑一个喜欢的名字，而是在判断这些对象能否在一次公开分发或再发布中同时满足各自条件。Open Source Guides 对这一点的提醒很直接：强 Copyleft 依赖往往要求项目采用相同或兼容的许可证，而某些 `source-available` 材料甚至会让项目根本不能被称作标准意义上的开源。这里不必马上去背兼容矩阵，但至少应建立一个稳定判断：在决定对外发布之前，先确认这些材料能不能被放进同一个公共分发边界里。

压缩成日常工程里的最低合规动作，大致可以先做下面几步：

- 先看仓库根目录的 `LICENSE`
- 再看是否还有 `NOTICE`、README 中的许可说明或第三方许可列表
- 区分代码、文档、图片、数据、模型等不同对象，不默认它们共享同一许可证
- 对来源不明、无许可证或条款不清楚的材料，先停下来核查，而不是先用再说
- 只有在确认使用、修改、分发和公开发布边界之后，才继续复用、混合或发布

如果项目准备对外发布衍生版本，还应再多问一步：自己是否需要继续沿用原项目名称、标识或品牌表达。如果需要，这通常已经不只是代码许可证问题，而可能进入商标边界。很多开源项目欢迎代码被复用，却不欢迎下游在容易引发混淆的情况下继续使用原有项目名义对外发布；这一点越到项目成熟、社区规模越大时越重要。
...
- Apache Software Foundation, “Apache Software Foundation Trademark Policy”
