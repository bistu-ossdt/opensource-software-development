...
需要补充的是，OSI 近年还把这类“什么才算开源”的讨论进一步扩展到了人工智能系统（`AI system`）。按照开源人工智能定义（`Open Source AI Definition`）的口径，判断一个人工智能系统是否属于开源人工智能（`Open Source AI`），不能只看模型参数能否下载，而要看支撑修改与再分发所需的数据信息、代码和参数是否一并达到开放条件。也正因为如此，开放权重（`Open Weights`）并不等于开源人工智能；许多公众语境里被笼统称作“开源模型”的对象，更准确地说往往只是开放权重或一般开放发布。本章在这里先把这条边界立住，后面关于项目阅读、参与路径和发布责任时还会再次回到它。
